...
async def _latest(db: aiosqlite.Connection, target_id: int) -> dict | None:
    async with db.execute(
        "SELECT ok, rtt_ms, status, ts FROM probe_result WHERE target_id = ? ORDER BY ts DESC LIMIT 1",
        (target_id,),
    ) as cur:
        row = await cur.fetchone()
    if not row:
        return None
    return {"ok": bool(row["ok"]), "rtt_ms": row["rtt_ms"], "status": row["status"], "ts": row["ts"]}


async def _history(db: aiosqlite.Connection, target_id: int) -> list[dict]:
    async with db.execute(
        f"SELECT ok, rtt_ms, ts FROM probe_result WHERE target_id = ? ORDER BY ts DESC LIMIT {_HISTORY_POINTS}",
        (target_id,),
    ) as cur:
